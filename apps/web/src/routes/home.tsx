    ...
    <main className="mx-auto max-w-5xl px-6 pb-10 pt-6">
      <div className="grid gap-4 lg:grid-cols-[1.1fr_1fr]">
        <section className="rounded-2xl border border-sky-100 bg-white p-5 shadow-sm">
          <h1 className="text-2xl font-bold text-slate-800">Google / Microsoft ログイン</h1>
        ...
        <section className="rounded-2xl border border-sky-100 bg-white p-5 shadow-sm">
          <h2 className="mb-3 text-sm font-semibold text-slate-600">受信トレイ(サンプル)</h2>
          <div className="space-y-2">
            {mailItems.map((mail) => (
              <article
                key={`${mail.sender}-${mail.time}`}
                className={`rounded-lg border px-3 py-2 ${mail.active ? 'border-slate-300 bg-slate-50' : 'border-slate-100 bg-white'}`}
              >
                ...
                <span className={`mt-1 inline-block rounded px-1.5 py-0.5 text-[10px] font-semibold ${mail.tag.color}`}>{mail.tag.text}</span>
              </article>
            ))}
          </div>
        </section>
